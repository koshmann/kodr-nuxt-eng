<template>
	<header
		id="header"
		class="w-full relative top-0 z-10"
		:class="{'stuck': stuck}"
	>
		<div
			id="masthead"
		...
		>
			<div
				class="relative overflow-hidden bg-gray-light grid grid-cols-3 justify-between items-center p-3 m-3 rounded z-10 transition-colors duration-300"
				:class="{'!bg-white': stuck}"
			>
				<div
					class="absolute w-full h-full top-0 left-0 bg-white rounded opacity-0 transform -translate-x-full transition duration-200 ease-in-out"
				...
				<div class="hidden lg:flex flex-row items-center z-10">
					...
					<nav class="flex flex-row">
						<nuxt-link
							...
							:class="{'transform translate-y-4 rotate-6 origin-left opacity-0 transition-all duration-200 delay-75': !menuActive}"
						>
							Проекты
						</nuxt-link>
						<nuxt-link
